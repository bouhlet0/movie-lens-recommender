...
class Recommender:
    ...
    def from_imdb_ids(
        self, imdb_ids: list[int], k: int = 10, wildcard: bool = False, pool_size: int = 75
    ) -> pl.DataFrame:
    ...
    def from_imdb_csv(
        self, csv_path: str, min_rating: float = 7.0, k: int = 10, wildcard: bool = False, pool_size: int = 75
    ) -> pl.DataFrame:
        ...
        return self._recommend(item_indices, k, weights=weights, wildcard=wildcard, pool_size=pool_size)

    def from_titles(
        self, titles: list[str], k: int = 10, wildcard: bool = False, pool_size: int = 75
    ) -> pl.DataFrame:
        item_indices = self._resolve_titles(titles)
        if not item_indices:
    ...
    def _recommend(
        ...
        proxy_user = self._build_proxy_user(item_indices, weights)
        scores = self.model._item_factors @ proxy_user
        scores[item_indices] = -np.inf

        if wildcard:
            effective_pool = min(pool_size, int(np.isfinite(scores).sum()))
            pool = np.argsort(-scores)[:effective_pool]
            chosen = np.random.choice(pool, size=min(k, effective_pool), replace=False)
            top_k = chosen[np.argsort(-scores[chosen])]
        else:
            top_k = np.argsort(-scores)[:k]

        return self._build_output(top_k, k)

    def _build_proxy_user(
        self, item_indices: list[int], weights: list[float] | None
    ) -> np.ndarray:
        factors = self.model._item_factors[item_indices]
        if weights is not None and len(weights) == len(item_indices):
            w = np.array(weights, dtype=np.float32)
            w = w / w.sum()
            return (factors * w[:, None]).sum(axis=0)
        return factors.mean(axis=0)

    def _build_output(self, top_k: np.ndarray, k: int) -> pl.DataFrame:
    ...
    def _imdb_ids_to_item_indices_weighted(
        self, imdb_ids: list[int], ratings: list[float]
    ...
    def _resolve_titles(self, titles: list[str]) -> list[int]:
        item_indices = []
        for title in titles:
    ...
    def _title_search(
        self, title: str, top_n: int = 5, min_score: float = 60.0
    ) -> pl.DataFrame:
        ...
        matched_titles = [
            raw_titles[normalized_titles.index(m[0])]
            for m in matches if m[1] >= min_score
        ]
    ...
    def _empty_result() -> pl.DataFrame:
        return pl.DataFrame(
            schema={"rank": pl.Int64, "title": pl.Utf8, "genres": pl.Utf8, "imdb_url": pl.Utf8}
        )